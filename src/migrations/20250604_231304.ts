...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`announcements_featured_image_idx\` ON \`announcements\` (\`featured_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`departments_contacts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`departments_contacts_order_idx\` ON \`departments_contacts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`departments_contacts_parent_id_idx\` ON \`departments_contacts\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`departments_contacts_photo_idx\` ON \`departments_contacts\` (\`photo_id\`);`)
  await db.run(sql`CREATE TABLE \`departments_quick_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`departments_quick_links_order_idx\` ON \`departments_quick_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`departments_quick_links_parent_id_idx\` ON \`departments_quick_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`departments_services\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`departments_services_order_idx\` ON \`departments_services\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`departments_services_parent_id_idx\` ON \`departments_services\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`departments\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`overview\` text,
  	\`office_info_address\` text,
  	\`office_info_hours\` text DEFAULT 'Monday - Friday
  8:00 AM - 5:00 PM',
  	\`office_info_main_phone\` text,
  	\`office_info_fax\` text,
  	\`content\` text,
  	\`featured_image_id\` integer,
  ...
  await db.run(sql`CREATE INDEX \`departments_featured_image_idx\` ON \`departments\` (\`featured_image_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`departments_contacts\`;`)
  await db.run(sql`DROP TABLE \`departments_quick_links\`;`)
  await db.run(sql`DROP TABLE \`departments_services\`;`)
